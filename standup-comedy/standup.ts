...
const agent = new JokeTellerAgent()

agent.setInferenceVisitor(new TimeCounter())
const outputMessage = ModelMessageItem.rehydrate({
	text: "Why did the pigeon quit his job? Because he was tired of working for breadcrumbs.",
})
const messageText = "Tell me a joke about cars"
const message = UserMessageItem.create(messageText)
...
const context = ModelContext.create(projectId)
	.addContextItem(outputMessage)
	.addContextItem(developerMessage)
	.addContextItem(message)
